...
//前序遍历非递归版本
func TranverseCommon(root *TreeNode) {
	stack := make([]*TreeNode, 0)
	var preNode *TreeNode = root
	for preNode != nil || len(stack) != 0 {
		if preNode != nil {
...
//中序遍历
func TraverseMiddle(root *TreeNode) {
	stack := make([]*TreeNode, 0)
	var preNode = root
	for preNode != nil || len(stack) != 0 {
		if preNode != nil {
...
//后续遍历:非递归
func TranverseBehind(root *TreeNode) {
	stack := make([]*TreeNode, 0)
	var top *TreeNode
	var preNode *TreeNode = nil
	stack = append(stack, root)
	for root != nil && (preNode == nil || len(stack) != 0) {
		top = stack[len(stack)-1]
		if (top.Left == nil && top.Right == nil) || (preNode != nil && (preNode == top.Left || preNode == top.Right)) {
			fmt.Println(top.Val)
			stack = stack[:len(stack)-1]
			preNode = top
		} else {
